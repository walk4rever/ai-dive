...
- 文章高光分享：选中正文段落后弹出工具栏（AI解读 + 高光分享），点击「高光分享」复制分享文案（引用段落 + 标题引用 + 原文链接 + 品牌 slogan），适合社交媒体分享
- Agent 对话超时优化：后端超时从 90s 提升到 300s，支持多工具调用复杂场景；前端追踪流完整性，超时切断时显示 incomplete 提示并保留已回答内容
